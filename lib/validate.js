...
v.extend(validate, {
	...
	Promise: typeof _Promise !== "undefined" ? _Promise : /* istanbul ignore next */null,
	...
	// output format.
	processValidationResults: function processValidationResults(results, options) {
		var errors = {};
	...
	fullMessages: function fullMessages(errors, options) {
		options = options || {};
	...
	numericality: function numericality(value, options) {
		...
		for (name in checks) {
			...
			if (v.isNumber(count) && !checks[name](value, count)) {
				...
				var msg = options.message || this["not" + v.capitalize(name)] || "must be %{type} %{count}";
